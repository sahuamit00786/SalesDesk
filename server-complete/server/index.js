...
import { ensureAurinkoSubscriptions } from './src/services/aurinko/aurinkoSyncService.js'
import { isAurinkoConfigured } from './src/services/aurinko/aurinkoEnv.js'
...
function startBackgroundJobs() {
  ...
  // Aurinko renews provider-side subscriptions itself; this daily pass only
  // re-creates subscriptions that were dropped (e.g. webhook endpoint was
  // temporarily unreachable during a deploy).
  if (isAurinkoConfigured()) {
    const aurinkoHealMs = Number(process.env.AURINKO_SUBSCRIPTION_HEAL_INTERVAL_MS || 86400000)
    setTimeout(() => {
      ensureAurinkoSubscriptions().catch(() => {})
    }, 20000)
    if (aurinkoHealMs > 0) {
      setInterval(() => {
        ensureAurinkoSubscriptions().catch(() => {})
