...
export async function getCalls(user, filters = {}) {
  ...
  if (filters.workspaceId) {
    ...
    // No explicit workspace selected → non-company-admins (incl. workspace
    // admins/managers) still only see calls inside THEIR workspaces.
    const allowed = await allowedWorkspaceIdsForUser(user)
    where.workspaceId = allowed.length ? { [Op.in]: allowed } : { [Op.eq]: null }
  }

  // Explicit owner filter (Calls page "Owner" dropdown, elevated roles only —
  // for rank-3 callers this simply ANDs with their own-visibility clause).
  if (filters.ownerUserId && /^[0-9a-fA-F-]{36}$/.test(String(filters.ownerUserId))) {
    where.ownerUserId = String(filters.ownerUserId)
  }
