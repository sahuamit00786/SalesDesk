  ...
  async up(queryInterface, Sequelize) {
    ...
    if (!tables.includes('web_forms')) {
      await queryInterface.createTable('web_forms', {
        ...
        border_radius: {
          type: Sequelize.ENUM('none', 'sm', 'md', 'lg'),
          allowNull: false,
          defaultValue: 'md',
        },
        thankyou_type: {
          type: Sequelize.ENUM('message', 'redirect'),
          allowNull: false,
        ...
        display_type: {
          type: Sequelize.ENUM('inline', 'popup', 'slidein'),
          allowNull: false,
          defaultValue: 'inline',
        },
        popup_trigger: {
          type: Sequelize.ENUM('exit_intent', 'time_delay', 'scroll_depth', 'button_click'),
          allowNull: false,
        ...
        popup_position: {
          type: Sequelize.ENUM('center', 'bottom-right', 'bottom-left', 'bottom-center'),
          allowNull: false,
          defaultValue: 'center',
        },
    ...
    await addIndexIfMissing(queryInterface, 'web_forms', ['workspace_id', 'status'], {
      name: 'web_forms_workspace_status_idx',
    })
    await addIndexIfMissing(queryInterface, 'web_forms', ['public_token'], {
      name: 'web_forms_public_token_idx',
      unique: true,
    })

    if (!tables.includes('web_form_fields')) {
      await queryInterface.createTable('web_form_fields', {
        id: { type: Sequelize.UUID, allowNull: false, primaryKey: true, defaultValue: Sequelize.UUIDV4 },
        form_id: {
          type: Sequelize.UUID,
          allowNull: false,
          references: { model: 'web_forms', key: 'id' },
          onDelete: 'CASCADE',
          onUpdate: 'CASCADE',
        },
        type: {
    ...
    await addIndexIfMissing(queryInterface, 'web_form_fields', ['form_id', 'order'], {
      name: 'web_form_fields_form_order_idx',
    })

    if (!tables.includes('web_form_submissions')) {
    ...
    await addIndexIfMissing(queryInterface, 'web_form_submissions', ['form_id', 'submitted_at'], {
      name: 'web_form_submissions_form_submitted_idx',
    })
  },
