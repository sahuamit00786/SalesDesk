...
import { sendAurinkoMessage, isAurinkoAuthError } from '../services/aurinko/aurinkoClient.js'
import { isAurinkoConfigured } from '../services/aurinko/aurinkoEnv.js'
...
export async function getGoogleEmailAuthStatus(req, res, next) {
  try {
    // Aurinko-first: when a "Continue with Google" (Aurinko) account exists,
    // the mailbox + calendar features report connected through it and the
    // legacy direct-Google token is ignored for status purposes.
    const aurinkoAccount = await aurinkoAccountForUser(req.user.companyId, req.user.id)
    if (aurinkoAccount) {
      const scopes = String(aurinkoAccount.scopes || '')
      return res.json({
        success: true,
        data: {
          connected: true,
          readMailbox: true,
          calendarConnected: /calendar/i.test(scopes) || scopes === '',
          email: aurinkoAccount.email || null,
          updatedAt: aurinkoAccount.updatedAt || null,
          gmailPushConfigured: Boolean(aurinkoAccount.emailSubscriptionId),
          provider: 'aurinko',
        },
        meta: {},
      })
...
export async function getGoogleEmailConnectUrl(req, res, next) {
  try {
    // "Continue with Google" now redirects through Aurinko's OAuth flow when
    // the Aurinko app is configured. Every existing button in the client hits
    // this endpoint, so they all pick up the Aurinko flow with no UI changes.
    if (isAurinkoConfigured()) {
      const { getConnectUrl: aurinkoConnectUrl } = await import('./aurinkoController.js')
      return aurinkoConnectUrl(req, res, next)
    }
...
/**
 * Send a lead email through Aurinko (Gmail via Aurinko). Mirrors the Gmail
 * path exactly: LeadEmail row + tracking pixel/click rewriting + lead
 * activity, plus an immediate 'sent' metadata row for the mailbox outbox.
 */
async function sendLeadEmailViaAurinko({ req, res, lead, value, toRecipients, ccRecipients, bccRecipients, accountRow }) {
  ...
  const emailRow = await LeadEmail.create({
    ...
    fromEmail: accountRow.email || req.user.email || null,
    direction: 'outbound',
    status: 'queued',
  ...
  try {
    ...
    const message = {
      subject: value.subject || '',
      body: trackedBodyHtml,
      from: accountRow.email
        ? { name: req.user.name || undefined, address: accountRow.email }
        : undefined,
      to: toRecipients.map((address) => ({ address })),
      ...(ccRecipients.length ? { cc: ccRecipients.map((address) => ({ address })) } : {}),
      ...(bccRecipients.length ? { bcc: bccRecipients.map((address) => ({ address })) } : {}),
      ...(value.threadId && !String(value.threadId).startsWith('single:')
        ? { threadId: String(value.threadId) }
        : {}),
    }
    const sent = await sendAurinkoMessage(accountRow.accessToken, message)
    const providerMessageId = sent?.id != null ? String(sent.id) : null
    const providerThreadId = sent?.threadId != null ? String(sent.threadId) : message.threadId || null
    ...
    // Outbox metadata row — stored immediately so the Sent tab lists it
    // without waiting for the webhook (which will dedupe on the same id).
    if (providerMessageId) {
      await upsertMessageMetadata(
        accountRow,
        {
          ...
          threadId: providerThreadId,
          subject: value.subject || '',
          from: { name: req.user.name || '', address: accountRow.email || req.user.email || '' },
          to: toRecipients.map((address) => ({ address })),
          cc: ccRecipients.map((address) => ({ address })),
          bcc: bccRecipients.map((address) => ({ address })),
          sysLabels: ['sent'],
          receivedAt: new Date().toISOString(),
          hasAttachments: (value.attachments || []).length > 0,
          bodySnippet: htmlToText(value.bodyHtml || '').slice(0, 255),
        },
        { source: 'send', lead },
      ).catch(() => {})
    }
    ...
    return res.status(201).json({ success: true, data: emailRow, meta: { provider: 'aurinko' } })
  } catch (sendError) {
    await emailRow.update({ status: 'failed', errorMessage: String(sendError?.message || sendError) })
    if (isAurinkoAuthError(sendError)) {
      await markAccountAuthError(accountRow, sendError)
      return res.status(401).json({
        success: false,
        error: { code: 'GOOGLE_TOKEN_INVALID', message: 'Google account connection expired. Please reconnect via Continue with Google.' },
      })
    }
...
export async function sendLeadEmail(req, res, next) {
  try {
    ...
    // Aurinko-first send path (Gmail send through Aurinko). Sent mail logs via
    // the same metadata-first pattern: a metadata row is written immediately
    // (folder 'sent'); full content is available on click like any other mail.
    const aurinkoAccount = await aurinkoAccountForUser(req.user.companyId, req.user.id)
    if (aurinkoAccount) {
      return await sendLeadEmailViaAurinko({
        req,
        res,
        lead,
        value,
