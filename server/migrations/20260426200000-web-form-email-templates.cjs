...
module.exports = {
  async up(queryInterface, Sequelize) {
    await queryInterface.createTable('web_form_email_templates', {
      ...
      variables: { type: Sequelize.JSON, allowNull: false, defaultValue: ['name', 'email', 'form_name', 'submission_date'] },
      created_at: { allowNull: false, type: Sequelize.DATE },
      updated_at: { allowNull: false, type: Sequelize.DATE },
    })
    await queryInterface.addIndex('web_form_email_templates', ['workspace_id', 'created_at'], { name: 'web_form_email_templates_workspace_created_idx' })
  },
