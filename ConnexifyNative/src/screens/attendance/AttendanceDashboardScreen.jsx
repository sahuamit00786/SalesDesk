      ...
      {weekDays.map((d, i) => {
        ...
        return (
          <View key={i} style={[s.day, isToday && s.today]}>
            <Text style={[s.dayLabel, isToday && s.todayText]}>{dayLabel}</Text>
            <Text style={[s.dateNum, isToday && s.todayText]}>{d.getDate()}</Text>
            <View style={[
              s.dot,
              dotColor ? { backgroundColor: dotColor } : { backgroundColor: theme.colors.border },
              isFuture && { opacity: 0 },
            ]} />
          </View>
        );
...
const weekS = (theme) => StyleSheet.create({
  ...
  dayLabel:  { fontSize: fontSize.xs, color: theme.colors.textMuted, fontWeight: fontWeight.medium },
  dateNum:   { fontSize: fontSize.sm, color: theme.colors.textPrimary, fontWeight: fontWeight.semibold },
  todayText: { color: theme.colors.primary, fontWeight: fontWeight.bold },
        ...
        <View style={s.card}>
          {checkedOut ? (
            <View style={s.doneRow}>
              <View style={[s.doneIcon, { backgroundColor: theme.colors.successBg }]}>
...
const styles = (theme, insets) => StyleSheet.create({
  ...
  menuBtn:    { padding: spacing.xs, borderRadius: borderRadius.md },
  ...
  card:       { backgroundColor: theme.colors.surface, borderRadius: borderRadius.xl, padding: spacing.base, marginBottom: spacing.md, ...shadows.card },
  cardTitle:  { fontSize: fontSize.base, fontWeight: fontWeight.semibold, color: theme.colors.textPrimary, marginBottom: spacing.sm },
  ...
  checkoutBtn:  { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: theme.colors.danger, borderRadius: borderRadius.md, paddingVertical: spacing.md, gap: spacing.sm, marginTop: spacing.md },
  ...
  doneIcon:     { width: 52, height: 52, borderRadius: 26, alignItems: 'center', justifyContent: 'center' },
  doneTitle:    { fontSize: fontSize.md, fontWeight: fontWeight.semibold, color: theme.colors.textPrimary },
  doneMeta:     { fontSize: fontSize.sm, color: theme.colors.textSecondary, marginTop: 2 },
  doneHours:    { fontSize: fontSize.sm, color: theme.colors.success, fontWeight: fontWeight.medium, marginTop: 2 },
