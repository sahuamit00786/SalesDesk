...
function SimpleFilterRow({ filters, onChange, onReset, onApply, users, stageOptions, isOpportunities }) {
  // Rank-3 users never see the Assigned To control — their list is
  // server-scoped to their own leads regardless of what they'd pick.
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Assigned To — elevated roles only */}
      {elevated && users.length > 0 ? (
        <div>
...
export function FilterBuilder({
  ...
  // Rank-3 (sales/telecaller/custom) lists are server-scoped to their own
  // leads — the Assigned To filter is hidden for them in BOTH modes.
  ...
  const visibleGroups = FIELD_GROUPS.map((g) => ({
    ...
    fields: g.fields.filter((f) => {
      ...
      if (f.id === 'assignedTo' && !isElevated) return false
      return true
    }),
  }))
                        ...
                        <button
                          ...
                          onClick={() => {
                            try {
                              ...
                              // Rank-3: strip assignee rules a colleague may have
                              // saved into the preset (server ANDs with own-leads
                              // scope regardless; this keeps the UI honest).
                              if (!isElevated) {
                                delete parsed.assignedTo
                                if (parsed.filterTree?.rules) {
                                  parsed.filterTree = {
                                    ...parsed.filterTree,
                                    rules: parsed.filterTree.rules.filter((r) => r?.field !== 'assignedTo'),
                                  }
                                }
                              }
                              onChange(parsed)
