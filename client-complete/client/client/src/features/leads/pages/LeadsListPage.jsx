...
export function LeadsListPage({ variant = "leads" }) {
  ...
  // Elevated = company_admin / workspace_admin / manager → sees other users'
  // data, so gets the assignee filter + workspace switcher. Rank-3 roles have
  // server-scoped (own-leads-only) lists — the filter would be meaningless.
  const isElevated = useIsElevated();
  const canSwitchWorkspace = isElevated;
  ...
  return (
    <PageShell fullWidth>
        ...
        <ListSearchToolbar
          ...
          actions={
            <>
              ...
              {/* Assignee quick filter — elevated roles only. Rank-3 users'
                  lists are already server-scoped to their own leads, so the
                  control is hidden rather than shown pre-locked. */}
              {isElevated && (
                <div className="relative shrink-0">
                  <select
                ...
                onClick={() =>
                  exportAndDownload(isOpportunities ? "opportunities" : "leads", {
                    filters: {
                      ...
                      ...(isElevated && filters.assignedTo?.length ? { assignedTo: filters.assignedTo } : {}),
      ...
      <BulkActionsBar
        ...
        onAssign={isElevated ? openBulkAssign : undefined}
