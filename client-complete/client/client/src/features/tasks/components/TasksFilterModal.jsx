...
export function TasksFilterModal({ open, onClose, filters, onApply }) {
  ...
  // Workspace-scoped list; skipped entirely for rank-3 users, whose task
  // list is already server-scoped to themselves.
  const isElevated = useIsElevated()
  const { data: teamData } = useAssignableUsersQuery(undefined, { skip: !open || !isElevated })
