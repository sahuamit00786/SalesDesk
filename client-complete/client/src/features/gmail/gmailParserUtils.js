...
/**
 * Parse a thread served by the Aurinko-backed mailbox endpoint
 * (meta.source === 'aurinko'). The server already normalizes messages to
 * { id, threadId, subject, from:{name,email}, to[], cc[], date(ISO),
 *   bodyHtml, bodyText, snippet, attachments:[{id,name,mimeType,sizeBytes}],
 *   isUnread } — this maps them into the exact shape parseGmailThread yields
 * so every downstream component works unchanged.
 */
export function parseAurinkoThread(rawThread) {
  const messages = (rawThread?.messages || [])
    .map((msg) => {
      ...
      return {
        id: msg.id,
        threadId: msg.threadId || rawThread.id,
        subject: msg.subject || '(No subject)',
        from: { name: fromName, email: msg.from?.email || '', initials: getInitials(fromName) },
        to: (msg.to || []).map((a) => ({
          name: a.name || a.address || a.email || '',
          email: a.address || a.email || '',
          initials: getInitials(a.name || a.address || a.email || ''),
        })),
        cc: (msg.cc || []).map((a) => ({
          name: a.name || a.address || a.email || '',
          email: a.address || a.email || '',
        ...
        bodyHtml,
        bodyText,
        snippet: decodeHtmlEntities(msg.snippet || bodyText.slice(0, 180)),
        attachments: (msg.attachments || []).map((att) => ({
          id: att.id,
          ...
          color: getFileColor(att.name || ''),
        })),
        isUnread: Boolean(msg.isUnread),
        labelIds: msg.isUnread ? ['UNREAD'] : [],
      }
    })
    .sort((a, b) => a.date.getTime() - b.date.getTime())
  ...
  const last = messages[messages.length - 1]
  return {
    threadId: rawThread?.id || first?.threadId || '',
    subject: first?.subject || '(No subject)',
    participants: getUniqueParticipants(messages),
    messages,
    lastMessage: last || null,
