...
export function LeadTaskDrawer({ open, onClose, leadId: leadIdProp, task, leadTitle, onSaved }) {
  ...
  // Rank-3 users create tasks for themselves; elevated roles can delegate.
  const isElevatedRank = useIsElevated()
  const currentUserId = useSelector((s) => s.auth.user?.id)
  const allAssignUsers = metaRes?.data?.users || []
  const assignUsers = isElevatedRank ? allAssignUsers : allAssignUsers.filter((u) => String(u.id) === String(currentUserId))
