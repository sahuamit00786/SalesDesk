...
export function CallsPage() {
  ...
  // Owner filter — elevated roles only (rank-3 lists are self-scoped anyway)
  const isElevated = useIsElevated()
  const [ownerFilter, setOwnerFilter] = useState('')
  const { data: ownersData } = useAssignableUsersQuery(undefined, { skip: !isElevated })
  const ownerOptions = Array.isArray(ownersData?.data?.items) ? ownersData.data.items : []
  ...
  const query = useMemo(() => {
    ...
    if (isElevated && ownerFilter) q.ownerUserId = ownerFilter
    ...
    return q
  }, [page, limit, dateRange, customDateStart, customDateEnd, callType, hasLead, outcome, ownerFilter, isElevated, durationMin, durationMax, sortBy, sortOrder])
    ...
    <>
      ...
      {isElevated ? (
        <select
          ...
          className="h-10 shrink-0 cursor-pointer appearance-none rounded-xl border border-surface-border bg-white px-3 text-xs font-semibold text-ink shadow-sm outline-none focus:border-brand-400"
        >
