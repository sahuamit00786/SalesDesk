...
import { useRoleRank, rankOfRoleKind } from '@/hooks/useRoleRank'
...
export function TeamPage() {
  ...
  // Hierarchy (mirrors server roleHierarchy.js): the viewer may only
  // create/assign roles of STRICTLY lower rank than their own. The server
  // re-checks every write; these filters just keep the dropdowns honest.
  const myRank = useRoleRank()
  const assignableRoles = useMemo(
    () => roles.filter((r) => (r.manageable !== undefined ? r.manageable : myRank < rankOfRoleKind(r.userRoleKind || 'custom'))),
    [roles, myRank],
  )
  const roleKindOptions = useMemo(
    () => COMPANY_USER_ROLE_KIND_ALL_OPTIONS.filter((o) => myRank < rankOfRoleKind(o.value)),
    [myRank],
  )
      ...
      },
      {
        ...
        renderCell: ({ row: u }) => (
          <div className="inline-flex gap-1" onClick={(e) => e.stopPropagation()}>
            <button
              type="button"
              disabled={userBusy || u.manageable === false}
              onClick={() => openAccessDrawer(u)}
              aria-label="Edit member access"
              title={u.manageable === false ? 'You cannot manage a user at or above your own level' : 'Edit role, workspaces, and profile'}
            ...
            {u.isActive ? (
              <button
                ...
                title={u.manageable === false ? 'You cannot manage a user at or above your own level' : 'Deactivate user'}
        ...
        renderCell: ({ row: r }) => (
          <div className="inline-flex gap-1" onClick={(e) => e.stopPropagation()}>
            <button
              ...
              title={r.isDefault ? 'Default role cannot be edited' : r.manageable === false ? 'This role is at or above your level' : 'Edit role'}
              className="inline-flex h-7 w-7 items-center justify-center rounded-lg border border-brand-200 bg-white text-brand-700 shadow-sm hover:bg-white disabled:cursor-not-allowed disabled:opacity-50"
              disabled={r.isDefault || r.manageable === false}
              onClick={() => {
            ...
            <button
              ...
              title={r.isDefault ? 'Default role cannot be deleted' : r.manageable === false ? 'This role is at or above your level' : 'Delete role'}
              className="inline-flex h-7 w-7 items-center justify-center rounded-lg border border-red-200 bg-red-50 text-danger hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-50"
              disabled={deletingRole || r.isDefault || r.manageable === false}
              onClick={() => {
                setDeleteRoleDialog({
                  open: true,
                  roleId: r.id,
          ...
          <div>
            ...
            <div className="relative mt-1.5">
              ...
              >
                ...
                {roles
                  .filter((r) => assignableRoles.some((a) => a.id === r.id) || r.id === (accessUser?.companyRole?.id ?? ''))
                  .map((r) => (
                    <option key={r.id} value={r.id}>
                      {r.name}
                      {r.userRoleKind ? ` — ${labelCompanyUserRoleKind(r.userRoleKind)}` : ''}
