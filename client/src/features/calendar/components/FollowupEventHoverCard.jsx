...
function humanizeFollowupStatus(s) {
  const map = {
    pending: 'Pending',
    done: 'Done',
    cancelled: 'Cancelled',
...
export function FollowupEventHoverCard({ event, anchorRect, onMouseEnter, onMouseLeave, exiting = false, onExitTransitionEnd }) {
  ...
  const quickPick = event?.meta?.quickPickMinutes
  const remark = event?.meta?.remark?.trim()
    ...
    >
      <div className="flex items-start justify-between gap-2 border-b border-emerald-100 bg-gradient-to-r from-emerald-50 to-teal-50 px-3 py-2.5">
        <div className="min-w-0 flex-1">
          <span className="inline-flex rounded-md bg-emerald-600/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-emerald-900 ring-1 ring-emerald-200">
            Follow-up
          </span>
      ...
      <div className="scrollbar-subtle min-h-0 flex-1 space-y-3 overflow-y-auto p-3">
        <div className="overflow-hidden rounded-lg border border-emerald-100 bg-gradient-to-br from-emerald-50/90 via-white to-teal-50/50 p-3 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1 leading-tight">
              <p className="text-[11px] font-semibold text-gray-800">Scheduled call-back</p>
              <p className="text-[10px] text-gray-500">Stay on top of this lead touchpoint</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            {humanizeFollowupStatus(event?.status)}
          </span>
          {quickPick != null && [5, 10, 15].includes(Number(quickPick)) ? (
            <span className="rounded-md bg-gray-100 px-2 py-0.5 text-[11px] font-medium text-gray-700 ring-1 ring-gray-200">
              {Number(quickPick)} min preset
            </span>
          ) : null}
        </div>
        ...
        {event?.leadName && leadId ? (
          <Link
            to={`/leads/${leadId}`}
            className="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-2.5 py-2 text-xs font-medium text-emerald-700 hover:bg-emerald-50/60"
          >
