  ...
  return (
    <button
      ...
      className={`flex w-full items-start gap-3 border-b border-surface-border border-l-2 px-3 py-3 text-left transition-colors ${
        isActive ? 'border-l-blue-600 bg-blue-50/60' : 'border-l-transparent hover:bg-slate-50'
      }`}
    >
      <div className={`mt-2 h-1.5 w-1.5 rounded-full ${thread.isUnread ? 'bg-brand-500' : 'bg-transparent'}`} />
      <div className={`flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full text-xs font-semibold ${isActive ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-700'}`}>
        {senderInitials}
      </div>
...
export default function GmailThreadList({ threads, selectedId, onSelect }) {
  return (
    <div className="flex h-full w-[340px] min-w-[300px] max-w-[380px] flex-shrink-0 flex-col border-r border-surface-border bg-white">
      ...
      <div className="border-b border-surface-border px-3 py-2">
        <label className="relative block">
          ...
          <input
            ...
            className="h-9 w-full rounded-lg border border-surface-border bg-slate-50 pl-8 pr-2 text-xs text-ink-muted outline-none"
          />
        </label>
      </div>
