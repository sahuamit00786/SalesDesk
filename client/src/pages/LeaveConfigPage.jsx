...
export function LeaveConfigPage() {
  ...
  return (
    <PageShell fullWidth mainClassName="!py-0">
      <div className="w-full space-y-0 px-4 pb-6 pt-4 sm:px-6 sm:pt-6">
        <div className="mb-4 rounded-2xl border border-brand-200/60 bg-gradient-to-r from-brand-50/80 via-white to-white px-4 py-3.5 shadow-sm ring-1 ring-black/[0.04] sm:px-5">
          <p className="text-sm text-ink-muted">
            Configure leave types, employee balances, weekly offs, and public holidays. Changes apply to leave calculations
            for your workspace.
          </p>
        </div>

        <div className="overflow-hidden rounded-2xl border border-surface-border bg-white shadow-sm ring-1 ring-black/[0.04]">
          <div className="flex flex-wrap items-center gap-2 border-b border-surface-border bg-surface-subtle/30 px-3 py-2.5 sm:px-4">
            {TABS.map((tab) => {
              const Icon = tab.icon
            ...
            {activeTab === 'calendar' ? (
              <div className="space-y-6">
                ...
                <div className="rounded-xl border border-surface-border bg-white p-4 sm:p-5">
                  <div className="mb-4 flex items-start gap-3 border-b border-surface-border/70 pb-4">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-brand-200/60 bg-slate-100 text-brand-700 shadow-sm">
                      <Clock className="h-5 w-5" />
                    </span>
                    <div>
                      <h2 className="text-base font-semibold tracking-tight text-ink">Attendance rules</h2>
